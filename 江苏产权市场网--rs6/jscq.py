...
class JSCQ:
    def __init__(self):
        self.headers = {
            "Referer": "https://www.jscq.com.cn/",
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36"
        }
        self.url = 'https://www.jscq.com.cn/'
    ...
    def get_cookie(self):
        ...
        functionCode = self.requests.get(
            'https://www.jscq.com.cn' + etree.HTML(response.text).xpath('//script[2]/@src')[0],
            headers=self.headers).text
        with open('./jscq.js', encoding='utf-8') as f:
            jsCode = f.read().replace("'content'", content).replace("'tsCode'", tsCode).replace("'functionCode'",
                                                                                                functionCode)
        cookie = execjs.compile(jsCode).call('get_cookie').split('=')
        return cookie
